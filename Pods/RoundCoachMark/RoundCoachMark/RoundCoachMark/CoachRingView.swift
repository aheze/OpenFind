...
class CoachRingView: UIView, CAAnimationDelegate {
    ...
    private func animateEcho(_ open: Bool) {
        guard let ring = ringGeometry else { return }
        let echo_timing = CAMediaTimingFunction(name: CAMediaTimingFunctionName.default)
        let opacity_timing = CAMediaTimingFunction(name: CAMediaTimingFunctionName.linear)
