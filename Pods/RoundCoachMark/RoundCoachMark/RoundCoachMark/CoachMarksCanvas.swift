...
class CoachMarksCanvas: UIView {
    ...
    func show(_ mark: CoachMarker.MarkInfo, completion: @escaping () -> Void) {
        ...
        (markInfo as! UIView).alpha = 0
        let ring = CoachRing(controlCenter: mark.position, controlRadius: mark.aperture / 2, innerRect: info_frame, outerRect: bounds) // , excenterShift:CGPoint(x:0,y:-20), excenterRadius:nil)
    ...
    var ringMainColor: UIColor = .init(red: 0.000, green: 0.387, blue: 0.742, alpha: 0.8)
    var ringEchoColor: UIColor = .white
